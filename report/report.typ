...
La práctica implementa un servicio de mensajería distribuido formado por cuatro
componentes: el servidor en C, el cliente en Python, un servicio web con Flask y 
un servidor de logging mediante ONC-RPC. Los componentes se comunican a través de 
sockets TCP, salvo la transferencia de ficheros, que es directamente P2P entre clientes.

== Arquitectura general

El protocolo entre cliente y servidor usa mensajes de texto delimitados por `#` 
y terminados en el byte nulo (`\0`). El servidor responde siempre con un byte de 
código de estado (`0` = éxito, `1`/`2`/`3` = tipos de fallo específicos), seguido 
opcionalmente de más cadenas. Cada operación abre una conexión TCP nueva, envía 
su mensaje y cierra, excepto CONNECT, que es especial.

El servidor acepta conexiones en un bucle principal y lanza un hilo POSIX (`pthread`) 
por cada una. Para evitar condiciones de carrera, todos los accesos a la base de 
datos están protegidos por un mutex global.

== Servidor (`server.c`)

=== Recepción y dispatch de mensajes

La función `recv_msg` lee el mensaje byte a byte hasta encontrar el `\0` y lo 
divide por `#` en un array de campos. El tamaño del buffer está dimensionado 
para acomodar el mensaje más largo posible, que es SENDATTACH con todos los 
campos al máximo. Después, `handle_client` compara el primer campo con los 
nombres de operación conocidos y llama al manejador correspondiente.

=== REGISTER y UNREGISTER

`handle_register` llama a `user_add` y devuelve el código de resultado 
directamente. `handle_unregister` hace lo propio con `user_remove`. En caso de 
éxito, ambas invocan `rpc_log` para registrar la operación.
...
`handle_disconnect` comprueba que la petición viene de la misma IP desde la 
que el usuario se conectó, comparando la IP guardada en la base de datos con 
la del socket entrante. Esto evita que un proceso externo desconecte a otro usuario.

=== SEND y SENDATTACH

Ambos manejadores funcionan igual: verifican que el destinatario existe 
(aunque esté desconectado), almacenan el mensaje en la base de datos 
con `msg_add` y responden al cliente con el identificador asignado. 
Luego intentan entregar el mensaje de inmediato mediante `conn_deliver`. 
Si la entrega falla (el receptor se desconectó), el mensaje queda pendiente 
en la base de datos hasta el próximo CONNECT.

`handle_send_attached` es idéntico a `handle_send` salvo que también lee 
el campo del nombre de fichero y lo pasa a `msg_add` y `conn_deliver`. Para 
SENDATTACH, `rpc_log` recibe además el nombre del fichero.

=== conn_deliver

Esta función centraliza la entrega de mensajes al receptor. Construye la 
cadena del protocolo adecuada: `SEND_MESSAGE#...` para mensajes de texto 
o `SEND_MESSAGE_ATTACH#...` para mensajes con fichero. Se conecta al socket 
de escucha del receptor y envía el mensaje. Si la conexión falla, marca al 
usuario como desconectado. Si tiene éxito, borra el mensaje de la base de 
datos con `msg_delete` y envía el ACK al emisor (`SEND_MESS_ACK` o 
`SEND_MESS_ATTACH_ACK` según el caso). El envío del ACK es de tipo best-effort: 
si el emisor ya no está conectado, simplemente se omite.

=== USERS

`handle_users` verifica que el usuario solicitante está conectado y devuelve 
la lista de usuarios conectados. Para cada uno, además del nombre, envía su 
IP y puerto de escucha en formato `nombre::IP::puerto`. Esta información es 
necesaria para que los clientes puedan iniciar transferencias P2P.

== Cliente (`commands.py`)

=== Hilo de escucha (\_listen\_server)

Al ejecutar CONNECT, el cliente reserva un puerto libre con `bind("", 0)`, 
envía la petición al servidor indicando ese puerto y arranca un hilo en segundo
 plano. Este hilo abre un socket de escucha en ese puerto y queda bloqueado en 
 un bucle de `accept` con un timeout de 10 segundos. Cuando llega una conexión, 
 lee el mensaje y lo pasa a `_parse_incoming_message`, que identifica el tipo 
 de notificación (mensaje de texto, mensaje con fichero, ACK de entrega) y lo 
 muestra por consola. El timeout permite que el hilo compruebe periódicamente 
 la variable `_terminate` para saber si debe salir del bucle.

=== SENDATTACH

El método `send_attach` verifica en primer lugar que la ruta del fichero sea 
absoluta antes de hacer nada. Si no lo es, devuelve un error directamente 
sin contactar al servidor. El protocolo enviado es `SENDATTACH#emisor#receptor#mensaje#fichero`.
...
para llegar al servidor), se usa un truco habitual con sockets UDP: se 
crea un socket DGRAM, se llama a `connect` con la dirección del servidor 
sin enviar ningún dato, y se lee la IP local del socket resultante. Así 
el SO selecciona la interfaz de red adecuada automáticamente.

=== Normalización de mensajes

Antes de enviar cualquier SEND, el cliente hace una petición POST al 
servicio web en `http://servidor:3000/quitar-espacios` con el texto del 
mensaje. Si el servicio responde con código 200, usa el texto normalizado. 
Si el servicio no está disponible o responde con error, el mensaje original 
se envía sin modificar. Este comportamiento de fallback es importante para 
que el cliente siga funcionando aunque el servicio web no esté activo.

== Capa de persistencia (`users.c`)

La base de datos SQLite tiene dos tablas. La tabla `users` almacena el 
nombre (clave primaria), el estado de conexión (0/1), la IP, el puerto de 
escucha y un contador de mensajes. La tabla `messages` almacena los mensajes 
pendientes de entrega con sus campos: identificador numérico, emisor, 
receptor, texto y nombre de fichero (NULL para mensajes sin adjunto).

Los identificadores de mensaje son por receptor: cuando llega un mensaje 
nuevo, se incrementa el contador del receptor en la base de datos y el valor 
resultante se usa directamente como identificador del mensaje. Este diseño 
garantiza que el incremento y la lectura del ID son atómicos respecto al 
mutex. Los identificadores se envuelven de vuelta a 1 cuando llegan al 
máximo de `unsigned int`.

== Servicio web (`src/ws/app.py`)

El servicio está implementado con Flask y expone un único endpoint: 
`POST /quitar-espacios`. Recibe un JSON con el campo `cadena`, divide 
el texto en palabras y las vuelve a unir con un único espacio entre 
cada una, eliminando así cualquier secuencia de espacios en blanco 
redundante. Devuelve el resultado como texto plano con código 200. 
El servidor escucha en `0.0.0.0:3000` para ser accesible desde otros 
contenedores o máquinas.

== Servidor RPC (`src/rpc/`)

La interfaz está definida en `log_rpc.x`. El programa tiene número de 
programa 100, versión 1, y ofrece un único procedimiento `registrar` 
que recibe una estructura con tres cadenas de hasta 256 caracteres: 
nombre de usuario, operación y nombre de fichero. El servidor imprime 
esta información por consola al recibir cada llamada.

La función `rpc_log` del cliente RPC lee la variable de entorno 
`LOG_RPC_IP`. Si no está definida o está vacía, no hace nada. Si está 
definida, crea un cliente RPC con `clnt_create`, llama al procedimiento 
remoto y destruye el cliente. Los fallos son siempre silenciosos para 
no interrumpir el flujo normal de la aplicación.

Los ficheros `log_rpc_clnt.c`, `log_rpc_svc.c` y `log_rpc_xdr.c` 
fueron generados con `rpcgen` a partir del fichero `.x` y no se han 
modificado. Se compilan con solo `-O3` (sin `-Wall -Wextra -Werror`) 
para evitar que los warnings del código generado automáticamente rompan la compilación.
...
El comando `make` compila el servidor de mensajería y el servidor RPC. 
...
otra para el cliente y fuimos ejecutando los comandos a mano, comprobando 
que las respuestas que aparecían por pantalla coincidían con lo esperado. 
Repetimos esto para los distintos escenarios: registros, conexiones, envíos 
en línea y en diferido, adjuntos, transferencia de ficheros y el servidor RPC. 
En total ejecutamos 133 casos repartidos en 18 bloques, y todos dieron el 
resultado correcto. En la siguiente tabla recogemos los más representativos 
...
La parte más complicada de implementar fue el protocolo de 
transferencia de ficheros P2P. Al principio no teníamos claro 
cómo el cliente solicitante podía saber la IP y el puerto del otro 
cliente, hasta que nos dimos cuenta de que había que modificar USERS 
para que devolviera esa información junto con el nombre de usuario. 
Una vez entendido eso, el flujo en sí (el cliente solicitante manda 
un mensaje GETFILE directamente al socket de escucha del otro, que abre 
el fichero y lo envía en chunks) fue bastante directo de implementar.

Otro problema que tardamos bastante en encontrar fue un bug de doble 
incremento en la función que asigna identificadores a los mensajes. 
El contador del receptor se incrementaba dos veces: una en el `UPDATE` 
de SQLite y otra al calcular el nuevo ID en el código C, lo que producía 
IDs 2, 4, 6... en lugar de 1, 2, 3. Lo detectamos al escribir los tests de 
SENDATTACH, donde los IDs esperados no coincidían. La solución fue usar 
directamente el valor que ya devolvía el `SELECT` tras el primer `UPDATE`, 
sin volver a incrementar.

El servicio web fue lo más rápido de hacer, Flask es muy cómodo para esto. 
El único detalle que había que tener en cuenta era que escuchara en `0.0.0.0` 
y no solo en `localhost`, para que fuera accesible desde otros contenedores. 
En el cliente también fue importante el fallback silencioso: si el servicio 
no está disponible, el mensaje se manda igual sin espacios normalizados, para no 
bloquear el resto de la funcionalidad.

La parte del RPC fue interesante pero algo tediosa. El flujo de `rpcgen` 
genera stubs con código C antiguo que produce bastantes warnings con 
`-Wall -Wextra`, así que hubo que compilarlos por separado sin esas 
...
fallos parciales (qué pasa si el receptor se cae justo cuando le mandas 
un mensaje), coordinación entre varios procesos independientes con 
diferentes tecnologías de comunicación (sockets, HTTP, RPC) y 
persistencia para no perder mensajes. Todo eso junto hace que 
la práctica sea bastante representativa de un sistema de producción 
real, aunque simplificado.
